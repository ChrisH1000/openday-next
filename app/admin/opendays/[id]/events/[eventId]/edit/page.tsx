...
export default function Page({ params }: { params: Promise<{ id: string; eventId: string }> }) {
  ...
  const { data: events, error, isLoading } = useSWR<Event[]>(`/api/opendays/${resolvedParams.id}/events`, fetcher);
  ...
  const event = events?.find((eventItem) => eventItem.id === resolvedParams.eventId);
