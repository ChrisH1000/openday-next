...
export async function PUT(req: NextRequest, { params }: { params: { id: string; eventId: string; sessionId: string } }) {
  ...
  try {
    const updated = await updateSession({ ...data, id: params.sessionId });
...
export async function DELETE(req: Request, { params }: { params: { id: string; eventId: string; sessionId: string } }) {
  try {
    const result = await sql`DELETE FROM session WHERE id = ${params.sessionId} RETURNING *`;
